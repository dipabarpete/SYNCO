...
class DoctorService {
  ...
  Future<Doctor?> getDoctor(String doctorId) async {
    try {
      ...
      debugPrint('[DOCTOR_PORTAL] Warning: error getting doctor $doctorId: $e');
      return null;
    }
  }
  ...
  Future<Appointment> bookAppointment({
    ...
    await NotificationService().saveAppNotification(
      ...
      subtitle: 'New appointment request from $patientName for $date at $time.',
  ...
  Future<void> updateAppointmentStatus(String bookingId, String status) async {
    ...
    if (status == 'confirmed') {
      ...
      updates['declinedAt'] = FieldValue.serverTimestamp();
    ...
    final appointment = await getAppointment(bookingId);
    if (appointment != null) {
      ...
      if (status == 'confirmed') {
        await NotificationService().saveAppNotification(
          ...
          subtitle:
              'Your appointment with $doctorName on ${appointment.formattedDateShort} at ${appointment.slot} has been accepted.',
          iconCode: 0xe86c, // check_circle
          iconColorHex: 'FF45B69C', // mintGreen
          payload: 'appointment:$bookingId',
        );
        if (doctorId.isNotEmpty) {
          await NotificationService().saveAppNotification(
            ...
            subtitle:
                'You accepted the appointment request from ${appointment.patientName} for ${appointment.formattedDateShort} at ${appointment.slot}.',
            iconCode: 0xe86c,
        ...
        await NotificationService().saveAppNotification(
          ...
          subtitle:
              'Your appointment request with $doctorName was declined.',
        ...
        await NotificationService().saveAppNotification(
          ...
          subtitle: 'Your consultation with $doctorName has ended.',
          iconCode: 0xe86c,
          iconColorHex: 'FF45B69C',
          payload: 'appointment:$bookingId',
        );
        if (doctorId.isNotEmpty) {
          await NotificationService().saveAppNotification(
            ...
            title: 'Consultation Completed',
            subtitle:
                'Your consultation with ${appointment.patientName} has ended.',
            iconCode: 0xe86c,
            iconColorHex: 'FF45B69C',
            payload: 'appointment:$bookingId',
          );

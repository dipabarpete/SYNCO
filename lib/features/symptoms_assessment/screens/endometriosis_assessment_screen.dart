...
import 'endometriosis_assessment_result_screen.dart';

class EndometriosisAssessmentScreen extends ConsumerWidget {
  const EndometriosisAssessmentScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(endometriosisAssessmentProvider);
    final notifier = ref.read(endometriosisAssessmentProvider.notifier);

    // If screening is completed, navigate directly to Result Screen
    if (state.isCompleted && state.result != null) {
      return EndometriosisAssessmentResultScreen(result: state.result!);
    }

    final currentQuestion = state.currentQuestion;
    final totalQuestions = endometriosisQuestions.length;

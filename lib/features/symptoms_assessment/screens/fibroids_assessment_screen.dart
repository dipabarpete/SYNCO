...
import 'fibroids_assessment_result_screen.dart';

class FibroidsAssessmentScreen extends ConsumerWidget {
  const FibroidsAssessmentScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(fibroidsAssessmentProvider);
    final notifier = ref.read(fibroidsAssessmentProvider.notifier);

    // If screening is completed, navigate directly to Result Screen
    if (state.isCompleted && state.result != null) {
      return FibroidsAssessmentResultScreen(result: state.result!);
    }

    final currentQuestion = state.currentQuestion;
    final totalQuestions = fibroidsQuestions.length;
